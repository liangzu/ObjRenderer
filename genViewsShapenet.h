...
std::string vecFromDegree(int theta) {
    switch(theta) {
        case 0:
            return "";
        case 20:
        ...
        case 60:
            return "00100000000000000";
        case 80:
            return "00010000000000000";
        case 100:
            return "00001000000000000";
        case 120:
            return "00000100000000000";
        case 140:
            return "00000010000000000";
        case 160:
            return "00000001000000000";
        case 180:
            return "00000000100000000";
        case 200:
            return "00000000010000000";
        case 220:
            return "00000000001000000";
        case 240:
            return "00000000000100000";
        case 260:
            return "00000000000010000";
        case 280:
            return "00000000000001000";
        case 300:
...
void processModel(const std::string& rootPath,
                  ...
                  const Args& args)
{
    std::string modelPath = rootPath+filePath;
    char cmd0[1024];
//    sprintf(cmd0, "rm %s", (modelPath.substr(0, modelPath.length()-4)+".binvox").c_str());
//    system(cmd0);
//    sprintf(cmd0, "rm %s", (modelPath.substr(0, modelPath.length()-4)+"_1.binvox").c_str());
//    system(cmd0);
//    sprintf(cmd0, "rm %s", (modelPath.substr(0, modelPath.length()-4)+"_2.binvox").c_str());
//    system(cmd0);
//    sprintf(cmd0, "rm %s", (modelPath.substr(0, modelPath.length()-4)+"_3.binvox").c_str());
//    system(cmd0);

//    sprintf(cmd0, "binvox -d 32 %s", modelPath.c_str());
//    system(cmd0);
        ...
        const Args& args)
{
    ...
    for(int theta = 0; theta < 360; theta += args.theta_inc)
    {
        for(int phi = 0; phi <= args.phi_max; phi += args.phi_inc)
        {
            ...
            //sprintf(fn, "%s/%d_%d.png", viewFolderPath.c_str(), theta, phi);
            sprintf(fn, "%s/%d_%d.png", viewFolderPath.c_str(), phi, theta);
            cv::imwrite(fn, aa_image*255.0);
...
findModelsInFolder(const std::string& root, std::vector<std::pair<std::string, std::string> >& pathList)
{
    ...
    while ((dirp = readdir(dp)) != NULL) {
        ...
        if(path == "." || path == "..")
            continue;
        if(int(dirp->d_type) == 4) // directory
        {
